     ...
     def init_weights(self,init_type="normal",gain=0.02):
         def init_func(m) :
             ...
             if 'BatchNorm2d' in classname :
                 ...
                 elif init.type == "kaiming":
                     init.kaiming_normal_(m.weight.data,a=0,mode="fan_in")
                 elif init.type == "orthogonal":
...
class SegGenerator(BaseNetwork) :
    def __init__(self,opt,input_nc,output_nc=13,norm_layer=nn.InstanceNorm2d):
        super(SegGenerator,self).init__()
...
class FeatureExtraction(BaseNetwork):
    def __init__(self,input_nc,ngf=64,num_layers=4,norm_layer=nn.BatchNorm2d):
        ...
        for i in range(1,num_layers):
            nf_prev = nf
            nf = min(nf * 2,512)
            layers += [nn.Conv2d(nf_prev,nf,kernel_size=4,stride=2,padding=1),nn.ReLU(),norm_layer(512)]
...
class FeatureRegression(nn.Module):
    def __init__(self,input_nc=512,output_size =6,norm_layer=nn.BatchNorm2d):
        ...
        self.conv = nn.Sequential(nn.Conv2d(input_nc, 512, kernel_size=4, stride=2, padding=1), norm_layer(512), nn.ReLU(),
...
class TpsGridGen(nn.Module):
    def __init__(self,opt,dtype=torch.float):
        ...
        grid_X ,grid_Y = np.meshgrid(np.linspace(-0.9,0.9,opt.load_width),np.linspace(-0.9,0.9,opt.load_weight))
    ...
    def compute_L_inverse(self,X,Y):
        N = X.size()[0] # number of points along dimension 0
        #construct matrix K
        Xmat = X.expand(N,N)
        Ymat = Y.expand(N,N)
    ...
    def apply_transformation(self,theta,points):
        ...
        #compute weights for the non linear part
        ...
        #grid is expanded in point dim4
...
class MaskNorm(nn.Module):
    ...
    def forward(self,x,mask):
        ...
        return normalized_background + normalized_foreground
...
class ALIASResBlock(nn.Module):
    def __init__(self,opt,input_nc,output_nc,use_mark_norm=True):
    ...
    def forward(self,x,seg,misalign_mask=None):
        seg = F.interpolate(seg,seg=x.size()[2:],mode='nearest')
        if misalign_mask is not None:
            misalign_mask = F.interpolate(seg,seg=x.size()[2:],mode='nearest')

        x_s = self.shortcut(x,seg,misalign_mask)
        dx = self.conv_0(self.relu(self.norm_0(x, seg, misalign_mask)))
        dx = self.conv_1(self.relu(self.norm_0(x,seg,misalign_mask)))
        output = x_s + dx
        return output
...
class ALIASGenerator(BaseNetwork):
    def __init__(self,opt,input_nc):
        ...
        self.num.upsampling_layers =opt.num_upsampling_layers
        ...
        self.up_2 = ALIASResBlock(opt,nf * 4 + 16,nf * 2,use_mark_norm=False)
        self.up_3 = ALIASResBlock(opt,nf * 2 + 16,nf * 1,use_mark_norm=False)
        if self.num_upsampling_layers == 'most':
            self.up_4 = ALIASResBlock(opt,nf * 1 + 16,nf //2,use_mark_norm=False)
            nf = nf //2
